  ...
  it('owns and disposes its stylesheet in every theme', () => {
    for (const theme of ['dark', 'light']) {
      ...
      for (let pass = 0; pass < 2; pass++) {
        ...
        expect(document.querySelectorAll('#transcribe-plugin-styles')).toHaveLength(1)
        dispose()
        expect(document.querySelectorAll('#transcribe-plugin-styles')).toHaveLength(0)
      }
    }
  })
})
...
it('keeps package styles tokenized, scalable, and consistent with SDK drop markers', () => {
  ...
  hexes.delete('#3b82f6')
  const fixedGlyph = new RegExp('transcribe-search-icon')
